...
function formatFujifilmData($type, $tag, $intel, $data) {
	...
	if($type=="ASCII") {
		...
		if($tag=="1001") { //Sharpness
			if($data == 1) $data = plog_tr("Soft");
			else if($data == 2) $data = plog_tr("Soft");
			else if($data == 3) $data = plog_tr("Normal");
			else if($data == 4) $data = plog_tr("Hard");
			else if($data == 5) $data = plog_tr("Hard");
			else $data = plog_tr("Unknown").": ".$data;
		}
		if($tag=="1002") { //White Balance
			if($data == 0) $data = plog_tr("Auto");
			else if($data == 256) $data = plog_tr("Daylight");
			else if($data == 512) $data = plog_tr("Cloudy");
			else if($data == 768) $data = plog_tr("DaylightColor-fluorescence");
			else if($data == 769) $data = plog_tr("DaywhiteColor-fluorescence");
			else if($data == 770) $data = plog_tr("White-fluorescence");
			else if($data == 1024) $data = plog_tr("Incandescense");
			else if($data == 3840) $data = plog_tr("Custom");
			else $data = plog_tr("Unknown").": ".$data;
		}
		if($tag=="1003") { //Color
			if($data == 0) $data = plog_tr("Chroma Saturation Normal(STD)");
			else if($data == 256) $data = plog_tr("Chroma Saturation High");
			else if($data == 512) $data = plog_tr("Chroma Saturation Low(ORG)");
			else $data = plog_tr("Unknown: ").$data;
		}
		if($tag=="1004") { //Tone
			if($data == 0) $data = plog_tr("Contrast Normal(STD)");
			else if($data == 256) $data = plog_tr("Contrast High(HARD)");
			else if($data == 512) $data = plog_tr("Contrast Low(ORG)");
			else $data = plog_tr("Unknown: ").$data;
		}
		if($tag=="1010") { //FlashMode
			if($data == 0) $data = plog_tr("Auto");
			else if($data == 1) $data = plog_tr("On");
			else if($data == 2) $data = plog_tr("Off");
			else if($data == 3) $data = plog_tr("Red-Eye Reduction");
			else $data = plog_tr("Unknown: ").$data;
		}
		if($tag=="1020") { //Macro
		...
		if($tag=="1021") { //FocusMode
			if($data == 0) $data = plog_tr("Auto");
			else if($data == 1) $data = plog_tr("Manual");
			else $data = plog_tr("Unknown: ").$data;
		}
		...
		if($tag=="1031") { //PictureMode
			if($data == 0) $data = plog_tr("Auto");
			else if($data == 1) $data = plog_tr("Portrait");
			else if($data == 2) $data = plog_tr("Landscape");
			else if($data == 4) $data = plog_tr("Sports");
			else if($data == 5) $data = plog_tr("Night");
			else if($data == 6) $data = plog_tr("Program AE");
			else if($data == 256) $data = plog_tr("Aperture Priority AE");
			else if($data == 512) $data = plog_tr("Shutter Priority");
			else if($data == 768) $data = plog_tr("Manual Exposure");
			else $data = plog_tr("Unknown: ").$data;
		}
		...
		if($tag=="1300") { //BlurWarning
			if($data == 0) $data = plog_tr("No Warning");
			else if($data == 1) $data = plog_tr("Warning");
			else $data = plog_tr("Unknown: ").$data;
		}
		if($tag=="1301") { //FocusWarning
			if($data == 0) $data = plog_tr("Auto Focus Good");
			else if($data == 1) $data = plog_tr("Out of Focus");
			else $data = plog_tr("Unknown: ").$data;
		}
		if($tag=="1302") { //AEWarning
			if($data == 0) $data = plog_tr("AE Good");
			else if($data == 1) $data = plog_tr("Over Exposure");
			else $data = plog_tr("Unknown: ").$data;
		}
