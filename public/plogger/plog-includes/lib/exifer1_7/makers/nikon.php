...
function lookup_Nikon_tag($tag, $model) {
	...
	if($model==0) {
		switch($tag) {
			...
			case "000b": $tag = plog_tr("Converter"); break;
...
function formatNikonData($type, $tag, $intel, $model, $data) {
	...
	if($type=="ASCII") {
		...
		if($tag=="0003" && $model==0) { //Quality
			if($data == 1) $data = plog_tr("VGA Basic");
			else if($data == 2) $data = plog_tr("VGA Normal");
			else if($data == 3) $data = plog_tr("VGA Fine");
			else if($data == 4) $data = plog_tr("SXGA Basic");
			else if($data == 5) $data = plog_tr("SXGA Normal");
			else if($data == 6) $data = plog_tr("SXGA Fine");
			else $data = plog_tr("Unknown").": ".$data;
		}
		if($tag=="0004" && $model==0) { //Color
			if($data == 1) $data = plog_tr("Color");
			else if($data == 2) $data = plog_tr("Monochrome");
			else $data = plog_tr("Unknown").": ".$data;
		}
		if($tag=="0005" && $model==0) { //Image Adjustment
			if($data == 0) $data = plog_tr("Normal");
			else if($data == 1) $data = plog_tr("Bright+");
			else if($data == 2) $data = plog_tr("Bright-");
			else if($data == 3) $data = plog_tr("Contrast+");
			else if($data == 4) $data = plog_tr("Contrast-");
			else $data = plog_tr("Unknown").": ".$data;
		}
		...
		if($tag=="0007" && $model==0) { //White Balance
			...
			else if($data == 1) $data = plog_tr("Preset");
			else if($data == 2) $data = plog_tr("Daylight");
			else if($data == 3) $data = plog_tr("Incandescense");
			else if($data == 4) $data = plog_tr("Fluorescence");
			else if($data == 5) $data = plog_tr("Cloudy");
			else if($data == 6) $data = plog_tr("SpeedLight");
			else $data = plog_tr("Unknown").": ".$data;
		}
		if($tag=="000b" && $model==0) { //Converter
			if($data == 0) $data = plog_tr("None");
			else if($data == 1) $data = plog_tr("Fisheye");
			else $data = plog_tr("Unknown").": ".$data;
		}
		...
		if($tag=="0083" && $model==1) { //Lens Type
			...
			if($data == 0) $data = plog_tr("AF non D");
		...
		if($tag=="0087" && $model==1) { //Flash type
			$data = hexdec(substr($data, 0, 2));
			if($data == 0) $data = plog_tr("Did Not Fire");
			else if($data == 4) $data = plog_tr("Unknown");
			else if($data == 7) $data = plog_tr("External");
			else if($data == 9) $data = plog_tr("On Camera");
			else $data = plog_tr("Unknown").": ".$data;
		}
	}
...
function parseNikon($block, &$result) {
	...
	//these 6 models start with "Nikon". Other models don't.
